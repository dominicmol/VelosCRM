...
# generieke view voor gebruikersregistratie, open voor iedereen
class UserRegistrationView(generics.CreateAPIView):
    queryset = User.objects.all()
    serializer_class = UserCreateSerializer
    permission_classes = (AllowAny,)

    # overschrijf POST om beperkte response terug te geven
    def post(self, request, *args, **kwargs):
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        self.perform_create(serializer)
        headers = self.get_success_headers(serializer.data)
        # stuur alleen relevante velden terug na registratie
        response_data = {
...
# function-based view om het team van de ingelogde gebruiker op te halen
@api_view(['GET'])
@permission_classes([IsAuthenticated])
...
# function-based view om een nieuw lid aan het team toe te voegen
@api_view(['POST'])
@permission_classes([IsAuthenticated])
