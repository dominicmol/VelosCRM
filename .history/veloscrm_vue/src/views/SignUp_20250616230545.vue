...
export default {
  ...
  methods: {
    async submitForm() {
      ...
      try {
        // Aangepaste URL om overeen te komen met de Django backend route
        await axios.post('/api/v1/teams/signup/', { 
          username: this.email,
        ...
        if (error.response?.data) {
          for (const field in error.response.data) {
            // Pas aan om een array van foutmeldingen te verwerken als de backend die stuurt
            if (Array.isArray(error.response.data[field])) {
              error.response.data[field].forEach(msg => this.errors.push(`${field}: ${msg}`));
            } else {
